...
- [ ] **New Environment Variable Rule:**
    - **CRITICAL:** Environment variables MUST be defined in ALL THREE places:
        1. `config/configuration.ts` - for loading the config
        2. `config/validation.ts` - with `.required()` validation
        3. `example.env` - with a placeholder value
    - **CRITICAL:** If `process.env.VARIABLE_NAME` is used anywhere in the codebase, it MUST exist in all three places
    - **Violation Identity:** Variable in `validation.ts` but not in `configuration.ts` or `example.env`
    - **Violation Identity:** Variable in `example.env` but not in `configuration.ts` or `validation.ts`
    - **Violation Identity:** Any `process.env.VARIABLE_NAME` usage not defined in all three files
    - **Requirement:** All environment variables MUST have Joi validation (no defaults allowed)
- [ ] **Usage Pattern:**
    - **Violation Identity:** Direct usage of `process.env.VARIABLE_NAME` in application code
    - **Requirement:** Use `ConfigService.get()` to access environment variables (e.g., `configService.get('server.port')`)
    - **Exception:** Allowed in `config/configuration.ts` for loading configuration

## 10. File Update Procedures (Violation Handling)
...
- [ ] Env var in `validation.ts` but not in `configuration.ts` → Add to configuration.ts
- [ ] Env var in `example.env` but not in `validation.ts` → Add with `.required()` validation
- [ ] Env var used in code but missing from config files → Add to all three files
- [ ] Direct `process.env.VARIABLE_NAME` usage → Use `ConfigService.get()` instead
