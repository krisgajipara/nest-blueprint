...
- Location: `libs/@oc/server-core/database/entities/{module}.entity.ts`
- Inherit from `BaseModifiableEntity` (Multi-tenant with tenantId) or `BaseSystemModifiableEntity` (System-wide without tenantId)
- `BaseModifiableEntity` is the DEFAULT for all business modules
...
- [ ] **CRITICAL:** Entities MUST extend `BaseModifiableEntity` (unless strictly system-wide)
...
22. **Entity missing tenant context** → Extend `BaseModifiableEntity` instead of `BaseSystemModifiableEntity`
