...
- **Rule**: Every `BaseModifiableEntity` MUST have a dedicated `Repository` class extending `TenantAwareRepository`.
...
- **Entity Inheritance:**
    - All business entities **MUST** extend `BaseModifiableEntity` (from `@core-database`).
    ...
    // ✅ CORRECT
    @Entity()
    export class Tournament extends BaseModifiableEntity { ... }
    ```
